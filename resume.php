<?php  $title = 'Resume |'; include 'header.php'; ?>

<div id="page">
	<div id="content">
	    
		
		
		<div id="resume-intro">
		    <img src="img/portrait.png"/>
		    <h1>Christopher M. Kennedy</h1>
		    <h2>UX Strategist &amp; Mobile Designer<h2>
		<p>I am currently not looking for work. I am happily employed at M. Shanken Communications as their Lead UX Strategist. Cranken out iPad app, iPhone apps, Content Management Systems, internal tools, and public facing websites. I also keep myself extremely busy with a lot of side project, hackathons, organizing events, meetups,  and confrences. </p>
		</div><!--/resume-intro-->
		
		<hr/>
		
		<div id="resume">
		
		<div class="company">
		    <h2><a href="http://gradespotter.com">Grade Spotter</a></h2>
		    <p>December 2011 - Present</p>
		</div><!--/company-->
		<div class="details">
		<p>Co-Founder in New York, NY</p>
    		<ul>
    			<li>I am the Designer Co-Founder. I am in mainly in charge with designing the logo, iPhone user interface, and front-end development for the website.</li>
    		</ul>
		</div><!--/details-->
		
		<div class="company">
    		<h2><a href="http://www.mshanken.com">M. Shanken Communications</a></h2>
    		<p>March 2011 - Present</p>
		</div><!--/company-->
		<div class="details">
		    <p>Lead UX Strategist in New York, NY</p>
    		<ul>
    			<li>Responsible for diagramming workflow and production tools to support new products.</li>
    			<li>Create low-fidelity and high-fidelity wireframe concepts for new products.</li>
    			<li>Able to balance multiple projects over varying delivery dates.</li>
    			<li>Identify and create opportunities for user feedback in the development of new and existing features.</li>
    			<li>Self-motivated and able to produce results independently as well as in a team environment.</li>
    			<li>Passionate about the details while always keeping the big picture in mind.</li>
    			<li>Contribute to high-level strategic decisions with the rest of the product and executive teams.</li>
    		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://48hourapps.com">48HOURapps</a></h2>
		<p>March 2011 - Present</p>
		</div><!--/company-->
		<div class="details">
		    <p>Designer in New York, NY</p>
		<ul>
			<li>UX Designer &amp; Front-End Developer of web/tablet/mobile apps in 48 hours. I work with four awesome guys that love to hack on projects. We've built some pretty cool apps and people wanted us to build apps for them so we said... hey why not? </li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://www.hackerunion.org">Hacker Union</a></h2>
		<p>October 2011 - Present</p>
		</div><!--/company-->
		<div class="details">
		    <p>Root Member in New York, NY</p>
		<ul>
			<li>Helping support and grow New York's hacker culture.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://startupbus.com/">StartupBus NYC Hackathon</a></h2>
		<p>February 2012</p>
		</div><!--/company-->
		<div class="details">
		    <p>Organized in New York, NY</p>
		<ul>
			<li>I was tasked with organizing our traditional Pre-Boarding Hackathon.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://thehackerolympics.com/">Hacker Olympics</a></h2>
		<p>February 2012</p>
		</div><!--/company-->
		<div class="details">
		    <p>Attendee in New York, NY</p>
		<ul>
			<li>First hackathon I did not hack in. Helped with some very minor task to keep the show going.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://theeastwing.net/episodes/1-kennedy">The East Wing Podcast</a></h2>
		<p>February 2012</p>
		</div><!--/company-->
		<div class="details">
		    <p>Tooting My Own Horn</p>
		<ul>
			<li>The podcast talks with industry experts about design, solving problems and the keys to creating products with value. I was on the first show, episode 1. I talked a little about what I have been up to, M. Shanken Communcations, 48 Hour Apps, The StartupBus, and Grade Spotter.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://generalassemb.ly/local-hack-day">Reinvent Local</a></h2>
		<p>November 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Hacker in New York, NY</p>
		<ul>
			<li>Designed the overall look of the project for our team <a href="/projects/midpoint">Midpoint</a>. Spent most of my time hacking away in the terminal on the xml.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://photohackday.org">Photo Hack Day</a></h2>
		<p>August 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Hacker in New York, NY</p>
		<ul>
			<li>Designed the overall look of the project for our team <a href="/projects/voronoi-diacam">Voronoi Diacam</a>. I won the food eating contest and the team won best use of Flickr's API.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://open.blogs.nytimes.com/2011/08/19/recapping-times-open-html5-and-beyond/">TimesOpen - HTML5 and Beyond</a></h2>
		<p>August 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Attendee in New York, NY</p>
		<ul>
			<li>I had a front seat to listen to in-depth talks about HTML5 and related technologies. The speakers included <a href="http://twitter.com/#!/petele">Pete LePage</a>, <a href="http://twitter.com/#!/jashkenas">Jeremy Ashkenas</a> &amp; <a href="http://twitter.com/#!/davidpadbury">David Padbury</a>.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://ce1.com/intel/2011/nyappuplab/">NYC Intel AppUp Application Labs</a></h2>
		<p>August 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Attendee in New York, NY</p>
		<ul>
			<li>Learned a lot about MeeGo OS and the Intel AppUp Store. Discussed with a few friends and hope to have a MeeGo based project in the works.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://googlecode.blogspot.com/2011/07/mark-your-calendars-html5-hackathons.htm">Google's HTML5 Web App Hackathon</a></h2>
		<p>August 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Hacker in New York, NY</p>
		<ul>
			<li>Designed the logo and layout for our team <a href="/projects/transit-trainer">Transit Trainer</a>.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://forrst.com/">Forrst Meetup</a></h2>
		<p>July 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Organized in Brooklyn, NY</p>
		<ul>
			<li>I organized the secound NYC Forrst Meetup.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://disrupt.techcrunch.com/NYC2011/">TechCrunch Disrupt Conference</a></h2>
		<p>May 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Attendee in New York, NY</p>
		<ul>
			<li>It was nice to see a lot of highly respect individuals such as Tony Conrad, Ron Conway, Dennis Crowley, Paul Graham, Scott Harrison, Arianna Huffington, and much more.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://disrupt.techcrunch.com/NYC2011/hackathon/">TechCrunch Disrupt Hackathon</a></h2>
		<p>May 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Hacker in New York, NY</p>
		<ul>
			<li>Designed the logo and Android UI for our team <a href="/projects/use-my-crm">My CRM</a> in New York City.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://techcrunch.com/tag/lemonade-stand/">TechCrunched</a></h2>
		<p>April 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Tooting My Own Horn</p>
		<ul>
			<li>I have always looked up to TechCrunch and always wanted to be apart of it somehow. I never thought I would be on the front page.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2>Hack of the Month</h2>
		<p>April 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Tooting My Own Horn</p>
		<ul>
			<li>We where honored to be Hack Of The Month at <a href="http://www.meetup.com/ny-tech/">NY Tech Meetup</a> for our impressive technical achievements for Lemonade Stand. Check out the video of us on stage in front of over 770+ people for <a href="http://youtu.be/DC-AiiclmV0">Hack of the Month</a>.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://forrstpodcast.com/post/4253501707/forrst-podcast-88-dinosaurs-and-aliens-interview">Forrst Podcast</a></h2>
		<p>April 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Tooting My Own Horn</p>
		<ul>
			<li>The podcast covers the awesome Forrst community as well as the rest of the Internet and all things that are good in design and development. I was a guest on the 88th show, I talked a little about the StartupBus and some projects that I have been working on.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2>eBay Hangover Party and Speed Hack at SXSW</h2>
		<p>March 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Hacker in Austin, TX</p>
		<ul>
			<li>We integrated eBay's API in the newly created Lemonade Stand app for Android. We took home some pocket change for our hack.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2>Lemonade Stand</h2>
		<p>March 2011 - June 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Co-Founder in New York, NY</p>
		<ul>
			<li>We created this company on the <a href="http://www.startupbus.com/">StartupBus</a>. We had 48 hours to complete this project with a team of 5 others. I designed the logo and both Android &amp; iPhone UI for our team. All the hard worked help the team reach the semifinals, finishing in the top 12 of 38 teams. After SXSW we where celebrated as an impressive technical achievement at New York Tech Meetup's <a href="http://www.youtube.com/watch?v=DC-AiiclmV0&feature=youtu.be">Hack of the Month</a>. </li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://www.startupbus.com/">Startup Bus</a></h2>
		<p>March 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Buspreneur in New York, NY</p>
		<ul>
			<li>A bus of 25 strangers traveling at 60 miles an hour and over 48 hours on the road between New York, NY and Austin,TX attempted the challenge of starting a company under unique (and some would say extreme) time and resource constraints and produce something valuable. At the end we will present the project to a panel of high profile investors. Designed the logo and both Android &amp; iPhone UI for our team <a href="/projects/lemonade-stand">Lemonade Stand</a> while on a bus traveling from New York City to Austin, TX. All the hard worked help the team reach the semifinals, finishing in the top 12 of 38 teams. Fun Note, I was so excited I did not take my first nap until I hit 50 hours, I was nicked named <em>The Machine</em>.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://mlb.com">Major League Baseball</a></h2>
		<p>April 2010 - March 2011</p>
		</div><!--/company-->
		<div class="details">
		    <p>Marketing Producer in New York, NY</p>
		<ul>
			<li>Optimized overall performance of team sites in order to increase traffic, registered users and revenue generation</li>
			<li>Coordinated with design, editorial, and technical teams during new feature/functionality development projects</li>
			<li>Implemented site wide form template using HTML5 &amp; CSS3</li>
			<li>Designed and developed all Fan Club pages using CSS3</li>
			<li>Developed an internal documentation for standards and guidelines</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://www.ccny.cuny.edu/">City College of New York</a></h2>
		<p>September 2010 - December 2010</p>
		</div><!--/company-->
		<div class="details">
		    <p>Web Design Teacher in New York, NY</p>
		<ul>
			<li>Taught introduction course to web design</li>
			<li>Developed curriculum and class projects </li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://forrst.com/">Forrst Meetup</a></h2>
		<p>November 2010 in Brooklyn, NY</p>
		</div><!--/company-->
		<div class="details">
		    <p>Organized in New York, NY</p>
		<ul>
			<li>I organized the first NYC Forrst Meetup.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://go.liverfoundation.org/">Liver Life Walk</a></h2>
		<p>June 2010</p>
		</div><!--/company-->
		<div class="details">
		    <p>Charity in New York, NY</p>
		<ul>
			<li>Raise a total of $1,050 for the Liver Life Walk to show support for my kid brother. Donations raised to sponsor the walk provided research, education, and advocacy to assist the 30 million people affected by liver disease in the United States.</li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://www.cityofflaglerbeach.com">Flagler Beach Ocean Rescue</a></h2>
		<p>May 2000 - September 2006</p>
		</div><!--/company-->
		<div class="details">
		    <p>Head Lieutenant in Flagler Beach, FL</p>
		<ul>
			<li>Trained as Emergency Medical Technician (E.M.T.)</li>
			<li>Able to stay calm during adverse situations</li>
			<li>Broad knowledge of the ocean and ocean life</li>
			<li>Skills and knowledge to prevent and respond to emergencies</li>
			<li>Director of the Junior Lifeguard program</li>
			<li>Lead Instructor of Surf Lessons </li>
		</ul>
		</div><!--/details-->
		
		<div class="company">
		<h2><a href="http://www.airforce.com/">United States Air Force</a></h2>
		<p>September 2003 - May 2005</p>
		</div><!--/company-->
		<div class="details">
		    <p>Crew Chief in TX &amp; WA</p>
		<ul>
			<li>Honorably Discharged</li>
			<li>Ribbons &amp; Medals: Air Force Training Ribbon, Global War on Terrorism Service Medal, Marksmanship, &amp; The AF Outstanding Unit Award</li>
			<li>Aerospace Maintenance Technician C18, C135, E3, VC25, and VC137</li>
			<li>Advised on problems maintaining, servicing, and inspecting aircraft and aerospace support equipment</li>
			<li>Interpreted and advised on maintenance procedures and polices to repair aircraft and support equipment</li>
			<li>Troubleshot and maintained aircraft structures, systems, components, and support equipment</li>
			<li>Test repaired components and test equipment; adjusted, aligned, rigged, and calibrated</li>
			<li>Reviewed maintenance forms, aircraft records, and reports to ensure complete documentation</li>
			<li>In charge of twenty seven KC-135, each worth $13 million </li>
		</ul>
		</div><!--/details-->
		
		</div><!--/resume-->
		
		<div class="clear"></div>
		
	</div><!--/content-->
</div><!--/page-->

<?php include("footer.php"); ?>